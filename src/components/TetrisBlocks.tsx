...
interface TetrisBlocksDefinition {
  blocks: Block[];
  color: string;
}
...
export type TetrisBlocksType = keyof typeof TetrisBlocks;

export const TetrisBlocks: { [key: string]: TetrisBlocksDefinition } = {
  OrangeRicky: {
    blocks: [
      { x: 0, y: 0, z: 0 },
      { x: 1, y: 0, z: 0 },
  ...
  for (let x = 0; x < gridState.length; x++) {
    for (let z = 0; z < gridState[x].length; z++) {
      for (let y = 0; y < gridState[x][z].length; y++) {
        ...
        if (color) {
          cubes.push(<Cube key={`${x},${y},${z}`} position={{ x: x + 0.5, y: y + 0.5, z: z + 0.5 }} color={color} />);
        }
      }
    }
  }
